...
def train(args):
    ...
    if not args.skip_mvae:
        ...
        BaseTrainer(model=mvae, train_dataset=train_data, eval_dataset=test_data, training_config=trainer_config_mvae).train()
    ...
    if not args.skip_mmvae:
        ...
        BaseTrainer(model=mmvae, train_dataset=train_data, eval_dataset=test_data, training_config=trainer_config_mmvae).train()

    # --- MMVAE (Gaussian) ---
    if args.extra_gaussian_mmvae:
        print("\n--- Initializing MMVAE (Gaussian) ---")
        # Construct loss string from obj and looser args
        loss_type = args.obj
        if args.looser:
             if "looser" not in loss_type:
        ...
        mmvae_config_gaussian = MMVAEConfig(
            n_modalities=2,
            latent_dim=args.latent_dim,
            input_dims=input_dims,
            uses_likelihood_rescaling=use_rescaling,
        ...
        mmvae_gauss = MMVAE(mmvae_config_gaussian)
        ...
        BaseTrainer(model=mmvae_gauss, train_dataset=train_data, eval_dataset=test_data, training_config=trainer_config_gauss).train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--extra_gaussian_mmvae", action="store_true", help="Train an extra MMVAE model with Gaussian distributions")
